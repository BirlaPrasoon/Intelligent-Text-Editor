...
public class WPTree implements WordPath {
	...
	{
	    LinkedList<WPTreeNode> queue = new LinkedList<>();  //holds nodes to explore  
	    HashSet<String> visited = new HashSet<>(); //holds the visited strings
	    WPTreeNode curr=null;
	    
	    List<String> neighbours = new LinkedList<>();
	    // assign word1 to root node
	    root = new WPTreeNode(word1,null);
	    //add the initial word
	    visited.add(word1);
	    queue.add(root);
	    WPTreeNode child = null;
	    
	    while(!queue.isEmpty()) {
	    	printQueue(queue);
	    	curr = queue.remove(0);
	    	//get neighbours
	    	neighbours = nw.distanceOne(curr.getWord(), false);
	    	
	    	for(String n:neighbours) {
	    		if(!visited.contains(n)) {
	    			child = curr.addChild(n);
	    			visited.add(n);
	    			queue.add(child);
	    			
	    		}
	    		if(n.equals(word2)) {
    				System.out.println("Word Path: "+child.buildPathToRoot() + " ->" +child.getWord());
